...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        image, landmarks, image2 = sample['I'], sample['T'], sample['R']
        ...
        return {'I': image, 'T': landmarks, 'R': image2}


class SIRRDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.phase == 'train':
            self.natural_dir_A1 = os.path.join(opt.dataroot, opt.phase, 'real_T_1')
            self.natural_dir_A2 = os.path.join(opt.dataroot, opt.phase, 'real_R_1')
            self.natural_dir_B = os.path.join(opt.dataroot, opt.phase, 'real_I_1')
            self.natural_A1_paths = sorted(make_dataset(self.natural_dir_A1, opt.max_dataset_size))
            self.natural_A2_paths = sorted(make_dataset(self.natural_dir_A2, opt.max_dataset_size))
            self.natural_B_paths = sorted(make_dataset(self.natural_dir_B, opt.max_dataset_size))
            self.natural_size = len(self.natural_A1_paths)
            self.dir_A1 = os.path.join(opt.dataroot, opt.phase, 'syn_A1')
            self.dir_A2 = os.path.join(opt.dataroot, opt.phase, 'syn_A2')
        else:
            self.dir_A1 = os.path.join(opt.dataroot, opt.phase, 'generalization/real_T')
            self.dir_A2 = os.path.join(opt.dataroot, opt.phase + 'generalization/real_T')
            self.dir_B = os.path.join(opt.dataroot, opt.phase, 'generalization/real_I')
    ...
    def __getitem__(self, index):
        ...
        if self.opt.phase == 'train':
            if is_natural:
                ...
                A2_img = np.asarray(Image.open(A2_path).convert('RGB'))
                B_img = np.asarray(Image.open(B_path).convert('RGB'))
                imgs = self.crop({'I': B_img, 'T': A1_img, 'R': A2_img})
                A1_img, B_img, A2_img = Image.fromarray(imgs['T']), Image.fromarray(imgs['I']), Image.fromarray(imgs['R'])
                is_natural_int = 1
            else:
...
class TestDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.crop = RandomCrop(opt.load_size)
        self.dir_A1 = os.path.join(opt.dataroot, 'test', 'Nature/real_T')
        self.dir_B = os.path.join(opt.dataroot, 'test', 'Nature/real_I')
