...
def define_G(input_nc, output_nc, ngf, netG, device, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    if netG == 'resnet_9blocks':
        ...
        net = LRM(device=device)
        # net = gUNet(kernel_size=5, base_dim=16, depths=[16, 16, 16, 32, 16, 16, 16], conv_layer=ConvLayer,
...
def define_Cont(input_nc, ndf, netCont, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    """Create a discriminator
    ...
    if netCont == 'basic':  # default PatchGAN classifier
        net = NLayerDiscriminator_cont(input_nc, ndf, n_layers=n_layers_D, norm_layer=norm_layer)
    else:
        raise NotImplementedError('Discriminator model name [%s] is not recognized' % net)
    return init_net(net, init_type, init_gain, gpu_ids)
...
class PatchNCELoss(nn.Module):
    ...
    def forward(self, feat_q, feat_k, feat_v):
        ...
        batch_dim_for_bmm = self.batch_size // 8

        # reshape features to batch size, feat_q and feat_k: 1 x 256 x 256
        feat_q = feat_q.view(batch_dim_for_bmm, -1, dim)
        feat_v = feat_v.view(batch_dim_for_bmm, -1, dim)
        npatches = feat_q.size(1)
        l_neg_curbatch = torch.bmm(feat_q, feat_v.transpose(2, 1))
...
class four_margin_loss(nn.Module):
    ...
    def forward(self, tensor0, tensor3, labels, num, *tensor_mid):
        if num == 3:
            ...
            loss_I_T = self.margin_loss_L(tensor0, tensor3, labels)
            loss_I_fakeT = self.margin_loss_L(tensor0, tensor_mid[0], labels)
            loss_fakeT_T = self.margin_loss_S(tensor_mid[0], tensor3, labels)

            loss_R_I = self.margin_loss_L(tensor_mid[1], tensor0, labels)
            loss_fakeR_I = self.margin_loss_L(tensor_mid[2], tensor0, labels)
            loss_R_fakeR = self.margin_loss_S(tensor_mid[1], tensor_mid[2], labels)
            return loss_I_T + loss_I_fakeT + loss_fakeT_T + loss_R_I + loss_fakeR_I + loss_R_fakeR
...
class NLayerDiscriminator_cont(nn.Module):
    """Defines a PatchGAN discriminator"""
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        Parameters:
            ...
            norm_layer      -- normalization layer
        """
        super(NLayerDiscriminator_cont, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
...
class PatchSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        ...
        for mlp_id, input_nc in enumerate([3, 64, 128, 256, 512]):
...
class Prompts(nn.Module):
    def __init__(self, model, initials=None):
        ...
        if isinstance(initials, list):
            ...
            self.embedding_prompt = torch.nn.init.xavier_normal_(nn.Parameter(model.token_embedding(
                [" ".join(["X"] * config.length_prompt),
                 " ".join(["X"] * config.length_prompt)]).requires_grad_())).cuda()
        ...
        # self.meta_net2.half()
        #
        # init_weights(self.meta_net1, init_type='normal', init_gain=0.02)
        # init_weights(self.meta_net2, init_type='normal', init_gain=0.02)

    def forward(self, tensor, flag=1):
        tokenized_prompts = torch.cat([clip.tokenize(p) for p in [" ".join(["X"] * 16)]])
        # bias = self.meta_net1(tensor_R)  # (batch, ctx_dim)
        # bias = self.meta_net2(bias.T).T
        # bias = bias.unsqueeze(1)  # (batch, 1, ctx_dim)
        # bias = bias.repeat(1, 77, 1)
        ...
        for i in range(tensor.shape[0]):
            ...
            if flag == 0:
                ...
                if (i == 0):
                    probs = similarity.unsqueeze(0)
                else:
                    probs = torch.cat([probs, similarity.unsqueeze(0)], dim=0)
            else:
                similarity = (100.0 * image_features @ (text_features / nor).T).softmax(dim=-1)  # /nor
                if (i == 0):
                    probs = similarity[0:1]
                else:
                    probs = torch.cat([probs, similarity[0:1]], dim=0)
        return probs


def get_clip_score_from_feature(tensor, text_features, clip_model, prediction):
    ...
    for i in range(tensor.shape[0]):
        ...
        # prob = probs[0][0]
        if prediction == 1:
            score = score + probs[0][0] + probs[0][1]
        elif prediction == 0:
            score = score + probs[0][0] + probs[0][2]
...
class L_clip_from_feature(nn.Module):
    ...
    def forward(self, x, text_features, clip_model, prediction):
        k1 = get_clip_score_from_feature(x, text_features, clip_model, prediction)
        return k1


class L_clip_from_feature_R(nn.Module):
    def __init__(self):
        super(L_clip_from_feature_R, self).__init__()
        for param in self.parameters():
            param.requires_grad = False
    ...
    def forward(self, x, text_features, clip_model):
        ...
        for i in range(tensor.shape[0]):
            ...
            probs = similarity
            # prob = probs[0][0]
            score = score + probs[0][1] + probs[0][2]
        score = score / tensor.shape[0]
        return score


class L_prompt_NCE(nn.Module):
    def __init__(self):
        super(L_prompt_NCE, self).__init__()
        for param in self.parameters():
            param.requires_grad = False

    def forward(self, x_i, x_r, x_b, text_features, clip_model):
        clip_normalizer = transforms.Normalize((0.48145466, 0.4578275, 0.40821073),
                                               (0.26862954, 0.26130258, 0.27577711))
        img_resize = transforms.Resize((224, 224))
        score = 0
        ...
        tensor_i = x_i
        score_i = 0.
        for i in range(tensor_i.shape[0]):
            image2 = img_resize(tensor_i[i])
            image = clip_normalizer(image2.reshape(1, 3, 224, 224))
            ...
            probs = similarity
            # prob = probs[0][0]
            score_i = score_i + probs[0][0] + probs[0][2]
        score_i = score_i / tensor_i.shape[0]

        tensor_r = x_r
        score_r = 0.
        for i in range(tensor_r.shape[0]):
            image2 = img_resize(tensor_r[i])
            image = clip_normalizer(image2.reshape(1, 3, 224, 224))
            ...
            probs = similarity
            # prob = probs[0][0]
            score_r = score_r + probs[0][1] + probs[0][2]
        score_r = score_r / tensor_r.shape[0]

        tensor_b = x_b
        score_b = 0.
        for i in range(tensor_b.shape[0]):
            image2 = img_resize(tensor_b[i])
            image = clip_normalizer(image2.reshape(1, 3, 224, 224))
            ...
            probs = similarity
            # prob = probs[0][0]
            score_b = score_b + probs[0][0] + probs[0][1]
        score_b = score_b / tensor_b.shape[0]
        score = score_i + score_r + score_b
        return score

#
# class L_clip_encoders_feature(nn.Module):
#     def __init__(self):
#         super(L_clip_encoders_feature, self).__init__()
#         for param in self.parameters():
#             param.requires_grad = False
#
#     def forward(self, img_tensors, text_features, clip_model, labels):
#         clip_normalizer = transforms.Normalize((0.48145466, 0.4578275, 0.40821073),
#                                                (0.26862954, 0.26130258, 0.27577711))
#         img_resize = transforms.Resize((224, 224))
#         images = clip_normalizer(img_resize(img_tensors))
...
#         logits_per_image = (100 * image_features @ text_features.T).softmax(dim=-1)
#         logits_per_text = logits_per_image.t()
#
#         criterion_ce = nn.CrossEntropyLoss()
#         # image-axis loss
#         loss_img = criterion_ce(logits_per_image, labels)
#
#         # # text-axis loss
#         # labels = labels.t()
#         # tmp_loss = []
#         # for tmp_class_idx in range(3):
#         #     cur_tmp_loss = [logits_per_text[tmp_class_idx][labels == tmp_class_idx].mean().unsqueeze(0)]
#         #     for cur_tmp_inner_idx in range(3):
#         #         if cur_tmp_inner_idx == tmp_class_idx:
#         #             continue
#         #         cur_tmp_loss.append(logits_per_text[tmp_class_idx][labels == cur_tmp_inner_idx].mean().unsqueeze(0))
#         #     tmp_loss.append(torch.cat(cur_tmp_loss))
#         # loss_text = criterion_ce(torch.stack(tmp_loss), torch.zeros(3).long().to(labels.device))
#
#         # total loss
...
def get_clip_score_MSE(pred, inp, weight, res_model):
    score = 0
    clip_normalizer = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))
    img_resize = transforms.Resize((224, 224))
...
class L_clip_MSE(nn.Module):
    ...
    def forward(self, pred, inp, res_model, weight=[1.0, 1.0, 1.0, 1.0, 0.5]):
        res = get_clip_score_MSE(pred, inp, weight, res_model)
        return res


class L_img_cont(nn.Module):
    def __init__(self):
        super(L_img_cont, self).__init__()
        for param in self.parameters():
            param.requires_grad = False

    def forward(self, pred, gt, inp, refl, clip_model):
        score = 0
        clip_normalizer = transforms.Normalize((0.48145466, 0.4578275, 0.40821073),
                                               (0.26862954, 0.26130258, 0.27577711))
        img_resize = transforms.Resize((224, 224))
        for i in range(pred.shape[0]):
            pred_img = img_resize(pred[i])
            pred_img = clip_normalizer(pred_img.reshape(1, 3, 224, 224))
            pred_image_features = clip_model.encode_image(pred_img)

            inp_img = img_resize(inp[i])
            inp_img = clip_normalizer(inp_img.reshape(1, 3, 224, 224))
            inp_image_features = clip_model.encode_image(inp_img)

            gt_img = img_resize(gt[i])
            gt_img = clip_normalizer(gt_img.reshape(1, 3, 224, 224))
            gt_image_features = clip_model.encode_image(gt_img)

            refl_img = img_resize(refl[i])
            refl_img = clip_normalizer(refl_img.reshape(1, 3, 224, 224))
            refl_image_features = clip_model.encode_image(refl_img)

            cont_features = torch.cat([inp_image_features, gt_image_features, refl_image_features], dim=0)
            pred_nor = pred_image_features.norm(dim=-1, keepdim=True)
            cont_norm = cont_features.norm(dim=-1, keepdim=True)

            similarity = (100.0 * (pred_image_features / pred_nor) @ (cont_features / cont_norm).T).softmax(dim=-1)
            score = score + similarity[0, 0] + similarity[0, 2]

        score = score / pred.shape[0]
        return score
...
class Encoder(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats, csff):
        ...
        self.up12 = UpSample(n_feat, scale_unetfeats)
        ...
        if csff:
            self.csff_enc1 = UpSample(n_feat, 0)
            self.csff_enc2 = UpSample(n_feat + scale_unetfeats, 0)
            self.csff_enc3 = UpSample(n_feat + (scale_unetfeats * 2), 0)

            self.csff_dec1 = UpSample(n_feat, 0)
            self.csff_dec2 = UpSample(n_feat + scale_unetfeats, 0)
            self.csff_dec3 = UpSample(n_feat + (scale_unetfeats * 2), 0)
...
class ResNet(nn.Module):
    def __init__(self, n_feat, scale_resnetfeats, kernel_size, reduction, act, bias, scale_edecoderfeats, num_cab):
        ...
        self.conv_enc1 = UpSample(n_feat, scale_resnetfeats)
        self.conv_enc2 = UpSample(n_feat, scale_resnetfeats)
        self.conv_enc3 = UpSample(n_feat, scale_resnetfeats)

        self.conv_dec1 = UpSample(n_feat, scale_resnetfeats)
        self.conv_dec2 = UpSample(n_feat, scale_resnetfeats)
        self.conv_dec3 = UpSample(n_feat, scale_resnetfeats)
...
class PS(nn.Module):
    def __init__(self, n_feat, kernel_size, bias):
        ...
        self.up = nn.Sequential(conv(n_feat, n_feat * 4, kernel_size, bias=bias), nn.PReLU(),
                                  nn.PixelShuffle(2),
...
class MCDRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=40, scale_edecoderfeats=20, scale_resnetfeats=16, num_cab=6, kernel_size=3,
        ...
        self.stage1_decoder_img = Decoder(n_feat, kernel_size, reduction, act, bias, scale_edecoderfeats)
        self.stage1_imgconv = conv(n_feat, 3, kernel_size, bias=bias)
        self.stage1_decoder_R = Decoder(n_feat, kernel_size, reduction, act, bias, scale_edecoderfeats)
        self.stage1_Rconv = conv(n_feat, 3, kernel_size, bias=bias)

        self.stage2_encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_edecoderfeats, csff=True)
        self.stage2_decoder_img = Decoder(n_feat, kernel_size, reduction, act, bias, scale_edecoderfeats)
        self.stage2_imgconv = conv(n_feat, 3, kernel_size, bias=bias)
        self.stage2_decoder_R = Decoder(n_feat, kernel_size, reduction, act, bias, scale_edecoderfeats)
        self.stage2_Rconv = conv(n_feat, 3, kernel_size, bias=bias)

        self.stage3_orsnet = ResNet(n_feat, scale_resnetfeats, kernel_size, reduction, act, bias, scale_edecoderfeats,
                                    num_cab)

        self.nam12_img = NAM(n_feat, kernel_size=1, bias=bias, cha=3)
        self.nam12_R = NAM(n_feat, kernel_size=1, bias=bias, cha=3)
        self.nam23_img = NAM(n_feat, kernel_size=1, bias=bias, cha=3)
        self.nam23_R = NAM(n_feat, kernel_size=1, bias=bias, cha=3)

        self.ps12_img = PS(n_feat, kernel_size, bias=bias)
        self.ps12_R = PS(n_feat, kernel_size, bias=bias)
        self.ps23_img = PS(n_feat, kernel_size, bias=bias)
        self.ps23_R = PS(n_feat, kernel_size, bias=bias)

        self.concat12 = conv(n_feat * 3, n_feat, kernel_size, bias=bias)
        self.concat23 = conv(n_feat * 3, n_feat + scale_resnetfeats, kernel_size, bias=bias)
        self.tail = conv(n_feat + scale_resnetfeats, 3, kernel_size, bias=bias)

    def forward(self, x3_img, x2_img, x1_img, x2_R, x1_R):
        H = x3_img.size(2)
        W = x3_img.size(3)
        ...
        res1_img = self.stage1_decoder_img(feat1)

        ## Output results of Stage 1
        stage1_R = self.stage1_Rconv(res1_R[0]) + x1_R
        stage1_img = self.stage1_imgconv(res1_img[0]) + x1_img

        ## Apply Non-local Attention Module (NAM)
        x2_namfeats_R = self.nam12_R(res1_R[0], stage1_R)
        x2_namfeats_img = self.nam12_img(res1_img[0], stage1_img)

        x2_namfeats_R = self.ps12_R(x2_namfeats_R)
        x2_namfeats_img = self.ps12_img(x2_namfeats_img)
        ...
        ## Concatenate NAM features of Stage 1 with shallow features of Stage 2
        x2_cat = self.concat12(torch.cat([x2, x2_namfeats_img, x2_namfeats_R], 1))

        ## Process features through Encoder of Stage 2
        feat2 = self.stage2_encoder(x2_cat, feat1, res1_img)

        ## Pass features through Decoder of Stage 2
        res2_R = self.stage2_decoder_R(feat2)
        res2_img = self.stage2_decoder_img(feat2)

        ## Output results of Stage 2
        stage2_R = self.stage2_Rconv(res2_R[0]) + x2_R
        stage2_img = self.stage2_imgconv(res2_img[0]) + x2_img

        ## Apply NAM
        x3_namfeats_R = self.nam23_R(res2_R[0], stage2_R)
        x3_namfeats_img = self.nam23_img(res2_img[0], stage2_img)

        x3_namfeats_R = self.ps23_R(x3_namfeats_R)
        x3_namfeats_img = self.ps23_img(x3_namfeats_img)
        ...
        x3_cat = self.concat23(torch.cat([x3, x3_namfeats_img, x3_namfeats_R], 1))

        x3_cat = self.stage3_orsnet(x3_cat, feat2, res2_img)

        stage3_img = self.tail(x3_cat)

        return [stage1_img, stage2_img, stage3_img + x3_img], [stage1_R, stage2_R]
